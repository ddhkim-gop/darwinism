...
async function renderCurrentDraftGrades(picks, year) {
    ...
    const [values, keepers, nameMap, news, writeups] = await Promise.all([
        ...
        isCurrent ? fetch(`data/${year}/draft_news.json?v=202609230510`).then(r => r.ok ? r.json() : {items:[]}).catch(() => ({items:[]})) : Promise.resolve({items:[]}),
        isCurrent ? fetch(`data/${year}/draft_writeups.json?v=202609230510`).then(r => r.ok ? r.json() : {teams:{}}).catch(() => ({teams:{}})) : Promise.resolve({teams:{}}),
    ]);
